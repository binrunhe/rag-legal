...
export function AnimatedCharacters({
  ...
  return (
    <div className="relative flex h-full w-full items-end justify-center">
      <div className="absolute bottom-20 right-16 hidden rounded-full bg-white/10 px-3 py-1 text-xs backdrop-blur md:block">
        {isTyping ? "正在输入..." : "欢迎回来"}
      </div>

      <div className="absolute left-1/2 top-14 -translate-x-1/2 rounded-xl border border-white/20 bg-white/10 px-4 py-2 backdrop-blur">
      ...
      <div className="relative mb-4 flex w-full max-w-[360px] items-end justify-center gap-6">
        <div
          className={cn(
            "flex size-24 items-center justify-center rounded-3xl border border-white/25 bg-black/20 transition-transform duration-300",
            isTyping && "-translate-y-1",
          )}
        >
      ...
      <div className="absolute bottom-0 left-1/2 w-[320px] -translate-x-1/2 rounded-2xl border border-white/20 bg-black/20 px-4 py-3 text-xs backdrop-blur">
        <div className="flex items-center gap-2">
          <Lock className="size-3.5" />
          <span>
            {showPassword
              ? `密码可见，长度 ${passwordLength}`
              : `密码已隐藏，长度 ${passwordLength}`}
          </span>
        </div>
      </div>
    </div>
...
export function AnimatedCharacters({
  ...
  return (
    <div className="relative h-[400px] w-[550px] select-none">
      <div
        ref={purpleRef}
      ...
      <div
        ref={yellowRef}
        className="absolute bottom-0 right-0 transition-all duration-700 ease-in-out"
        style={{
          width: "130px",
          height: "220px",
